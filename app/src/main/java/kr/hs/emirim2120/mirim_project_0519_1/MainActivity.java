package kr.hs.emirim2120.mirim_project_0519_1;
...
public class MainActivity extends AppCompatActivity {
    LinearLayout linear_red, linear_yellow, linear_blue; //참조변수 선언

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        linear_red = findViewById(R.id.linear_red); //객체 참조값 대입
        linear_yellow = findViewById(R.id.linear_yellow);
        linear_blue = findViewById(R.id.linear_blue);
        Button btnRed = findViewById(R.id.btn_red); //객체를 참조변수에 대입
    ...
    View.OnClickListener btnListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            linear_red.setVisibility(View.INVISIBLE); //버튼 안 보이게 하기
            linear_yellow.setVisibility(View.INVISIBLE);
            linear_blue.setVisibility(View.INVISIBLE);
            switch (view.getId()){
                case R.id.btn_red:
                    linear_red.setVisibility(View.VISIBLE); // 버튼 보이게 하기
                    break;
